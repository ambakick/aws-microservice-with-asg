...
def run_test(name: str, check: Callable[[], None]) -> Tuple[str, bool, str]:
    try:
        check()
        return (name, True, "")
    except Exception as exc:  # noqa: BLE001 - explicit per-test failure capture
...
def main() -> int:
    ...
    tests: List[Tuple[str, Callable[[], None]]] = [
        (
            "GET /service1 returns expected message",
            lambda: assert_message(
                f"http://{alb_dns}/service1", "Hello from Service 1"
            ),
        ),
        (
            "GET /service2 returns expected message",
            lambda: assert_message(
                f"http://{alb_dns}/service2", "Hello from Service 2"
            ),
        ),
        (
            "ECR repository service1 exists",
            lambda: check_ecr_repository("service1", region),
        ),
        (
            "ECR repository service2 exists",
            lambda: check_ecr_repository("service2", region),
        ),
    ]
    ...
    failures = 0
    for name, passed, error_msg in results:
        if passed:
            print(f"[PASS] {name}")
        else:
            failures += 1
    ...
    print()
    if failures:
        print(f"Verification failed: {failures} test(s) failed.")
        return 1
